...
# مسار ملف القرار (يجب أن يكون الملف بنفس المجلد على جيت هب)
...
if not decree_system.full_text:
    ...
    if query:
        ...
        if matches:
            st.success(تم العثور على {len.matches if isinstance(matches, list) else len(matches)} نتيجة مطابقة:)
